...
def p_term_multiply(t):
    'term_arithmetique : term_arithmetique TIMES factor'
    RA4(t)
...
def check_variable_type(var_name, expected_type):
    #"""Vérifie que la variable est de type numérique (int ou float)."""
    if check_variable_declared(var_name) : # Vérifie si la variable est déclarée
...
def RA1(t):
    ...
    check_numeric(t[1])  # Vérifie que la variable est déclarée et numérique
...
def RA4(t):
    ...
    global quadruples
    if not isinstance(t[1],int) or not isinstance(t[1],float) :
     raise_semantic_error(t[1], "must be of type int or float.")
    elif not isinstance(t[3],int) or not isinstance(t[3],float):
     raise_semantic_error(t[1], "must be of type int or float.")
...
if __name__ == '__main__':
    ...
    expressions1 = [
        "INTEGER a = 5;",                   # Déclaration et affectation d'une variable
        "INTEGER b = 3;",                   # Déclaration et affectation d'une autre variable
        "INTEGER c;",               
        "c = a * b;"
        #"INTEGER d = a * b;",               # Expression avec multiplication
        #"INTEGER e = a + b * c;",           # Expression avec priorité des opérateurs (multiplication avant addition)
        #"INTEGER f = (a + b) * c;",         # Expression avec parenthèses
        #"INTEGER g = (a + b) * (c - d);",   # Expression plus complexe avec parenthèses
        #"INTEGER h = a / b;",               # Expression avec division
        #"INTEGER i = a - b + c;",           # Expression avec addition et soustraction
        #"INTEGER j = (a + b) / (c - d);"    # Expression avec des parenthèses dans la division

    ]
    for stmt in expressions1:
        print(f"Parsing statement: {stmt}")
        #parse_statement(stmt)
        parser_statement_debug(stmt)
        print("\nSymbol Table:")
