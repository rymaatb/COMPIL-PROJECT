...
def QUAD(op, arg1, arg2, result, quadruplets, qc):
    """Ajoute un quadruplet et incrémente le compteur."""
    quadruplets.append([op, arg1, arg2, result])
    return qc + 1

def parse_if_else(code):
    quadruplets = []
    qc = 0  # Réinitialisation locale du compteur de quadruplets

    # Analyse de la structure if-else if-else
    pattern_if_else_if = re.compile(
        r"if\s*\((.*?)\)\s*\{(.*?)\}(?:\s*else\s+if\s*\((.*?)\)\s*\{(.*?)\})*(?:\s*else\s*\{(.*?)\})?", 
        re.DOTALL
    )
    match = pattern_if_else_if.match(code)

    if match:
        # Extraction des blocs
        condition_then_pairs = []
        condition_then_pairs.append((match.group(1).strip(), match.group(2).strip()))  # Bloc `if`

        # Récupérer tous les `else if`
        for i in range(3, len(match.groups()) - 1, 2):
            if match.group(i):
                condition_then_pairs.append((match.group(i).strip(), match.group(i + 1).strip()))

        # Bloc `else`
        else_block = match.group(len(match.groups()))

        # Générer les quadruplets pour chaque condition
        br_indices = []  # Liste des indices pour les sauts finaux
        for condition, then_part in condition_then_pairs:
            start_bz = qc
            quadruplets.append(["BZ", condition, "", ""])  # Saut si la condition est fausse
            qc += 1

            # THEN block
            qc = parse_instruction(then_part, quadruplets, qc)

            br_indices.append(qc)  # Saut final après THEN
            quadruplets.append(["BR", "", "", ""])
            qc += 1

            # Mise à jour du BZ pour sauter au prochain bloc
            quadruplets[start_bz][2] = qc

        # ELSE block, s'il existe
        if else_block:
            qc = parse_instruction(else_block.strip(), quadruplets, qc)

        # Mise à jour des BR pour pointer à la fin
        for br_index in br_indices:
            quadruplets[br_index][1] = qc

    else:
        print("Erreur : syntaxe incorrecte dans if-else if-else.")

    return quadruplets, qc

def parse_instruction(instruction, quadruplets, qc):
    """
    Analyse une instruction simple et génère les quadruplets.
    """
    instruction = instruction.rstrip(";").strip()

    if "=" in instruction:
        left, expr = instruction.split("=")
        left = left.strip()
        expr = expr.strip()

        # Identifier l'opération (addition ou soustraction)
        if "+" in expr:
            var, val = map(str.strip, expr.split("+"))
            quadruplets.append(["+", var, val, left])
        elif "-" in expr:
            var, val = map(str.strip, expr.split("-"))
            quadruplets.append(["-", var, val, left])
        else:
            quadruplets.append(["=", expr, "", left])
        qc += 1

    return qc

def execute_quadruplets(quadruplets):
    """Exécute les quadruplets."""
    i = 0
    while i < len(quadruplets):
        op, arg1, arg2, result = quadruplets[i]

        if op == "BZ":
            # Évaluer la condition en injectant les variables
            condition = eval(arg1, {}, variables)
            if not condition:  # Si la condition est fausse
                i = int(arg2)  # Sauter au quadruplet de destination
                continue
        elif op == "+":
            variables[result] = variables.get(arg1, 0) + int(arg2)
        elif op == "-":
            variables[result] = variables.get(arg1, 0) - int(arg2)
        elif op == "=":
            variables[result] = eval(arg1, {}, variables)
        elif op == "BR":
            i = int(arg1)  # Sauter à l'index indiqué par arg1
            continue
        i += 1   
# Build the parser
parser = yacc.yacc()
...
if __name__ == '__main__':
    ...
    # Code à analyser
    code1 = "if (a > 3) { a = a + 2; }"
    code2 = "if (b > 3) { a = a + 10; } else if (b == 3) { a = a - 5; } else { a = a + 3; }"

 # Analyse et exécution des blocs
    for idx, code in enumerate([code1, code2], start=1):
        print(f"\n--- Analyse et exécution du bloc {idx} ---")
        print(f"Code analysé : {code}")

        # Réinitialiser quadruplets pour ce bloc
        quadruplets, qc = parse_if_else(code)
